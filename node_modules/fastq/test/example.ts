...
function worker(taskStatus: any, cb: fastq.done) {
  cb(null, 'hello ' + taskStatus)
}
...
function genericsWorker(this: GenericsContext, taskStatus: number, cb: fastq.done<string>) {
  cb(null, 'the meaning of life is ' + (this.base * taskStatus))
}

const queue2 = queueAsPromised(asyncWorker, 1)

async function asyncWorker(taskStatus: any) {
  return 'hello ' + taskStatus
}
