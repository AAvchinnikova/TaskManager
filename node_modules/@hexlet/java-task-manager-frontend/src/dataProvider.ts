...
export const dataProvider: MyDataProvider = {
  ...
  updateTaskStatus: async (source, destination) => {
    ...
    if (source.status === destination.status) {
      // moving taskStatus inside the same column

      const columnTasks = tasksByStatus[source.status];

      if (source.index > destination.index) {
        // taskStatus moved up, eg
        // dest   src
        //  <------
        // [4, 7, 23, 5]
        ...
        await Promise.all([
          ...
          ...columnTasks
            .filter(
              (taskStatus) =>
                taskStatus.index >= destination.index && taskStatus.index < source.index
            )
            .map((taskStatus) =>
              dataProvider.update("tasks", {
                id: taskStatus.id,
                data: { index: taskStatus.index + 1 },
                previousData: taskStatus,
              })
            ),
          // for the taskStatus that was moved, update its index
          dataProvider.update("tasks", {
            id: source.id,
      ...
      // moving taskStatus across columns
      ...
      await Promise.all([
        ...
        ...sourceColumn
          .filter((taskStatus) => taskStatus.index > source.index)
          .map((taskStatus) =>
            dataProvider.update("tasks", {
              id: taskStatus.id,
              data: { index: taskStatus.index - 1 },
        ...
        ...destinationColumn
          .filter((taskStatus) => taskStatus.index >= destination.index)
          .map((taskStatus) =>
            dataProvider.update("tasks", {
              id: taskStatus.id,
              data: { index: taskStatus.index + 1 },
        ...
        // change the dragged taskStatus to take the destination index and column
        dataProvider.update("tasks", {
          id: source.id,
          data: {
