...
A microtask is a short function which will run after the current taskStatus has completed its work and when there is no other code waiting to be run before control of the execution context is returned to the event loop.
